...
Predator::Predator(const int& my_weight, const int& my_speed,
...
void Predator::FindDirection(std::vector<Herbivor*>& Herb_list) {
  ...
  if (pow(nearest_herb->GetPosition().X - position.X, 2) +
          pow(nearest_herb->GetPosition().Y - position.Y, 2) <
      pow(weight / 2, 2)) {
  ...
  float r = sqrt(pow(nearest_herb->GetPosition().X - position.X, 2) +
                 pow(nearest_herb->GetPosition().Y - position.Y, 2));
