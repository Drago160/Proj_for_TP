...
Omnivor::Omnivor(const Cord& my_position)
    : Unit("omnivor", my_position),
      Predator(my_position),
      Herbivor(my_position) {
  reduce_power *= REDUCE_OMNI_KOOF;
}

Omnivor::Omnivor(const int& my_weight, const int& my_speed,
                 const int& my_variability, const _Color& my_color,
                 const Cord& my_position)
    : Unit(my_weight, my_speed, my_variability, my_color, my_position),
      Predator(my_position),
      Herbivor(my_position) {
  reduce_power *= REDUCE_OMNI_KOOF;
}

void Omnivor::FindDirection(std::vector<Food*>& Food_list,
                            std::vector<Herbivor*>& Herb_list) {
...
void Omnivor::Update(std::vector<Food*>& foods,
...
Omnivor* Omnivor::Born() {
  ...
  int y = position.Y + randSign() * sqrt(pow(weight / 5 + w / 5, 2) -
                                         pow((position.X - x), 2));
  int var = variability;
  if (rand_bool(10)) {
