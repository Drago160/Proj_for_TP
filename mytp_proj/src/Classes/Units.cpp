...
Unit::Unit(const int& my_weight, const int& my_speed, const int& my_variability,
           const _Color& my_color, const Cord& my_position) {
  weight = my_weight;
  speed = my_speed;
  color = my_color;
...
void Unit::SetEnergy() { energy = ENERGY; }

int Unit::GetEnergy() const { return energy; }

void Unit::SetPosition(const Cord& my_position) { position = my_position; }

Cord Unit::GetPosition() const { return position; }

void Unit::SetReducePower() {
  reduce_power = (int)(weight * (speed * speed) * ENERGY_KOOF);
}

void Unit::UpdateEnergy(const int& delta) { energy += delta; }

void Unit::Die() { delete this; }
