...
int label_height(int count)
{
	return StartHeight + (count-1)*DHeight;
}

//вызывает меню(возвращает флаги для некоторых действий в зависимости от нажатой кнопки
int menu(RenderWindow & window)
{
	...
	while (isMenu)
	{
		...
		if (IntRect(WINDOW_WEIGHT/2 - playWidth/2, label_height(1), playWidth, playHeight).contains(Mouse::getPosition(window)))
		{ menuPlay.setColor(Color::Red); menuNum = 1; }

		if (IntRect(WINDOW_WEIGHT/2 - resWidth/2, label_height(2), resWidth, resHeight).contains(Mouse::getPosition(window)))
		{ menuRestart. setColor(Color::Red); menuNum = 4; }

		if (IntRect(WINDOW_WEIGHT/2 - aboutWidth/2, label_height(3), aboutWidth, aboutHeight).contains(Mouse::getPosition(window))) 
		{ menuAbout.setColor(Color::Red); menuNum = 2; }

		if (IntRect(WINDOW_WEIGHT/2 - exitWidth/2, label_height(4), exitWidth, exitHeight).contains(Mouse::getPosition(window))) 
		{ menuExit.setColor(Color::Red); menuNum = 3; }
